...
TEST(LifecycleTest, DuplicateQueryTest)
{
    ...
    auto originalEnumerator = DefQuery::from(lis)
    ...
    // yet another copy, but this time from the first copy, because the original one has already ran down
...
{
    std::list<int> lis = { 1,6,3,8,9,2,7,5,4 };
    
    auto originalEnumerator = DefQuery::from(lis)
        .where([](const int a) { return a > 3 && a < 9; })
        .orderby([](const int left, const int right) { return left-right;});
    
    // As the source enumerator (originalEnumerator) is not an rvalue, but an lvalue, the enumerator stages from it are copied to the extendedEnumerator.
