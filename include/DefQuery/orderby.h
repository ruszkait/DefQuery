...
namespace DefQuery
{
	...
	orderby_enumerator<TDerived> enumerator<TValue, TDerived>::orderby(sorting_order order, const std::function<int(const value_type&, const value_type&)>& comparator) &&
	{
	...
	template<typename TSourceEnumerator>
	orderby_enumerator<TSourceEnumerator>& orderby_enumerator<TSourceEnumerator>::thenby(sorting_order order, const TComparator& comparator)
	{
