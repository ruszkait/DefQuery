...
class select_enumerator
	: public enumerator<
		  typename std::result_of<TProjection(const typename TSourceEnumerator::value_type&)>::type,
		  select_enumerator<TSourceEnumerator, TProjection>>
{
public:
	using TProjectedValue =
		typename std::result_of<TProjection(const typename TSourceEnumerator::value_type&)>::type;
...
private:
	...
	const TProjectedValue& current() const override
	{
		return this->operator*();
	}
...
select_enumerator<TDerived, TProjection>
enumerator<TValue, TDerived>::select(TProjection&& projector) &&
{
	return select_enumerator<TDerived, TProjection>(std::move(static_cast<TDerived&>(*this)),
													std::forward<TProjection>(projector));
}
...
enumerator<TValue, TDerived>::select(TProjection&& projector) &
{
	return select_enumerator<TDerived, TProjection>(static_cast<TDerived&>(*this),
													std::forward<TProjection>(projector));
}
...
template <typename TSourceEnumeratorConstr, typename TProjectionConstr>
select_enumerator<TSourceEnumerator, TProjection>::select_enumerator(
	TSourceEnumeratorConstr&& source, TProjectionConstr&& projector)
	: TBaseClass(false)
...
template <typename TSourceEnumerator, typename TProjection>
const typename select_enumerator<TSourceEnumerator, TProjection>::TProjectedValue&
select_enumerator<TSourceEnumerator, TProjection>::operator*() const
{
