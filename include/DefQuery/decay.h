...
namespace DefQuery
{
	...
	{
		auto instanceOnTheHeap = std::make_unique<TDerived>(std::move(static_cast<const TDerived&>(*this)));
	...
	template<typename TValue>
	decayed_enumerator<TValue>::decayed_enumerator(std::unique_ptr<enumerator_interface<TValue>>&& source)
		: TBaseClass(false)
