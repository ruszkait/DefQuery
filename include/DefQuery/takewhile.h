...
class takewhile_enumerator
	: public enumerator<typename TSourceEnumerator::value_type,
						takewhile_enumerator<TSourceEnumerator, TRunningCondition>>
{
public:
	...
	takewhile_enumerator(TSourceEnumeratorConstr&& source,
						 TRunningConditionConstr&& runningCondition);
...
takewhile_enumerator<TDerived, TRunningCondition>
enumerator<TValue, TDerived>::takewhile(TRunningCondition&& runningCondition) &&
{
	return takewhile_enumerator<TDerived, TRunningCondition>(
		std::move(static_cast<TDerived&>(*this)),
		std::forward<TRunningCondition>(runningCondition));
}
...
{
	return takewhile_enumerator<TDerived, TRunningCondition>(
		static_cast<TDerived&>(*this), std::forward<TRunningCondition>(runningCondition));
}
...
template <typename TSourceEnumeratorConstr, typename TRunningConditionConstr>
takewhile_enumerator<TSourceEnumerator, TRunningCondition>::takewhile_enumerator(
	TSourceEnumeratorConstr&& source, TRunningConditionConstr&& runningCondition)
	: TBaseClass(false)
...
takewhile_enumerator<TSourceEnumerator, TRunningCondition>::operator*() const
{
	return *_source;
}
