...
namespace DefQuery
{
	...
	public:
        ...
        template <typename TLeftEnumeratorConstr, typename TRightEnumeratorConstr>
		join_enumerator(TLeftEnumeratorConstr&& sourceLeft, TRightEnumeratorConstr&& sourceRight, const TKeyProjectionLeft& projectionKeyLeft,  const TKeyProjectionRight& projectionKeyRight,  const TMerging& merging);
        ~join_enumerator();
	...
	join_enumerator<TDerived, TRightEnumerator, TKeyProjectionLeft, TKeyProjectionRight, TMerging> enumerator<TValue, TDerived>::join(TRightEnumerator&& sourceRight, const TKeyProjectionLeft& keyProjectionLeft, const TKeyProjectionRight& keyProjectionRight, const TMerging& merging) &&
	{
        return join_enumerator<TDerived, TRightEnumerator, TKeyProjectionLeft, TKeyProjectionRight, TMerging>(std::move(static_cast<TDerived&>(*this)), std::forward<TRightEnumerator>(sourceRight), keyProjectionLeft, keyProjectionRight, merging);
	}
    ...
    join_enumerator<TDerived, TRightEnumerator, TKeyProjectionLeft, TKeyProjectionRight, TMerging> enumerator<TValue, TDerived>::join(TRightEnumerator&& sourceRight, const TKeyProjectionLeft& keyProjectionLeft, const TKeyProjectionRight& keyProjectionRight, const TMerging& merging) &
    {
        return join_enumerator<TDerived, TRightEnumerator, TKeyProjectionLeft, TKeyProjectionRight, TMerging>(static_cast<TDerived&>(*this), std::forward<TRightEnumerator>(sourceRight), keyProjectionLeft, keyProjectionRight, merging);
    }
    ...
    template <typename TLeftEnumerator, typename TRightEnumerator, typename TKeyProjectionLeft, typename TKeyProjectionRight, typename TMerging>
    template <typename TLeftEnumeratorConstr, typename TRightEnumeratorConstr>
	join_enumerator<TLeftEnumerator, TRightEnumerator, TKeyProjectionLeft, TKeyProjectionRight, TMerging>::join_enumerator(TLeftEnumeratorConstr&& sourceLeft, TRightEnumeratorConstr&& sourceRight, const TKeyProjectionLeft& projectionKeyLeft,  const TKeyProjectionRight& projectionKeyRight,  const TMerging& merging)
        : TBaseClass(false)
        , _sourceLeft(std::forward<TLeftEnumeratorConstr>(sourceLeft))
        , _sourceRightMaster(std::forward<TRightEnumerator>(sourceRight))
        , _keyProjectionLeft(projectionKeyLeft)
        , _keyProjectionRight(projectionKeyRight)
		, _merging(merging)
