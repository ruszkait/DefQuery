...
namespace DefQuery
{
	...
	public:
		...
		typename std::result_of<TAccumlatorInitializer(const TValue&)>::type aggregate(const TFolding& folder, const TAccumlatorInitializer& accumlatorInitializer);
		...
		orderby_enumerator<TDerived> orderby(sorting_order order, const std::function<int(const TValue&, const TValue&)>& comparator = [](const auto& left, const auto& right) { return left == right ? 0 : (left < right ? -1 : +1); }) &;
		orderby_enumerator<TDerived> orderby(sorting_order order, const std::function<int(const TValue&, const TValue&)>& comparator = [](const auto& left, const auto& right) { return left == right ? 0 : (left < right ? -1 : +1); }) && ;
		...
		join_enumerator<TDerived, TRightEnumerator, TKeyProjectionLeft, TKeyProjectionRight, TMerging> join(TRightEnumerator&& sourceRight, TKeyProjectionLeft&& keyProjectionLeft, TKeyProjectionRight&& keyProjectionRight, TMerging&& merging) &;
		template <typename TRightEnumerator, typename TKeyProjectionLeft, typename TKeyProjectionRight, typename TMerging>
		join_enumerator<TDerived, TRightEnumerator, TKeyProjectionLeft, TKeyProjectionRight, TMerging> join(TRightEnumerator&& sourceRight, TKeyProjectionLeft&& keyProjectionLeft, TKeyProjectionRight&& keyProjectionRight, TMerging&& merging) && ;
		...
		takewhile_enumerator<TDerived, TRunningConditionConstr> takewhile(TRunningConditionConstr&& runningCondition) &;
		template <typename TRunningConditionConstr>
		takewhile_enumerator<TDerived, TRunningConditionConstr> takewhile(TRunningConditionConstr&& runningCondition) && ;
