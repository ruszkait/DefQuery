...
namespace DefQuery
{
	...
	class groupby_enumerator : public enumerator<group<typename std::result_of<TKeyProjection(const typename TSourceEnumerator::value_type&)>::type, typename TSourceEnumerator::value_type>, groupby_enumerator<TSourceEnumerator, TKeyProjection>>
	{
	public:
		using TProjectedKey = typename std::result_of<TKeyProjection(const typename TSourceEnumerator::value_type&)>::type;
		using TBaseClass = enumerator<group<typename std::result_of<TKeyProjection(const typename TSourceEnumerator::value_type&)>::type, typename TSourceEnumerator::value_type>, groupby_enumerator<TSourceEnumerator, TKeyProjection>>;
	...
	private:
		...
		const group<TProjectedKey, typename TSourceEnumerator::value_type>& current() const override { return this->operator*(); }
	...
	groupby_enumerator<TDerived, TKeyProjection> enumerator<TValue, TDerived>::groupby(TKeyProjection&& keyProjector) &&
	{
		return groupby_enumerator<TDerived, TKeyProjection>(std::move(static_cast<TDerived&>(*this)), std::forward<TKeyProjection>(keyProjector));
	}
	...
	groupby_enumerator<TSourceEnumerator, TKeyProjection>::groupby_enumerator(TSourceEnumeratorConstr&& source, TKeyProjectionConstr&& keyProjection)
		: TBaseClass(false)
	...
	template <typename TSourceEnumerator, typename TKeyProjection>
	groupby_enumerator<TSourceEnumerator, TKeyProjection>::groupby_enumerator(const groupby_enumerator& other)
		: TBaseClass(other)
	...
	template <typename TSourceEnumerator, typename TKeyProjection>
	groupby_enumerator<TSourceEnumerator, TKeyProjection>& groupby_enumerator<TSourceEnumerator, TKeyProjection>::operator=(const groupby_enumerator& other)
	{
		if (&other == this)
			return this;
	...
	bool groupby_enumerator<TSourceEnumerator, TKeyProjection>::operator++()
	{
		...
		if (groupsMustBeInitialized)
		{
			while (++_source)
			{
				...
				auto foundIter = std::find_if(_groups.begin(), _groups.end(), [&elementKey](const auto& group) { return group._key == elementKey; });
	...
	template <typename TSourceEnumerator, typename TKeyProjection>
	const group<typename std::result_of<TKeyProjection(const typename TSourceEnumerator::value_type&)>::type, typename TSourceEnumerator::value_type>& groupby_enumerator<TSourceEnumerator, TKeyProjection>::operator*() const
	{
